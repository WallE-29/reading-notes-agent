...
4. **两层记忆，而不是一份日志。** Agent 维护一份只增不删的跨书偏好记忆，另外单独维护一份"读者画像"，随证据积累被重写。原始观察和综合出来的模型刻意分开存放：综合是一次判断，两者都保留才审计得动。提炼会去重（没有新信号就不写新条目），且会话异常结束时会在下次启动补做，Agent 对读者的模型不会悄悄丢掉。个人数据文件留在本地并被 gitignore。
